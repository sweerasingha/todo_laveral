...
class Images
{
    ...
    {
        $renamed_uploaded_file = $name ? FileFacade::makeGivenName($file, $name) : FileFacade::makeName($file);
...
{
    try {
        $driver = Config::get('images.driver', 'gd');
$manager = new ImageManager($driver);
        ...
        $thumbPath = $this->image_path . '/thumb/' . $size['width'] . 'x' . $size['height'];
        if (!File::exists($thumbPath)) {
            File::makeDirectory($thumbPath, 0755, true);
        }

        $image->fit($size['width'], $size['height'])->save($thumbPath . '/' . $file);
    } catch (\Exception $e) {
        Log::error("Error in saveThumb: " . $e->getMessage());
    }
}
    ...
    // public function saveThumb($file, $size_type)
    // {
    //     // $driver = Config::get('images.driver');

    //     // Correctly initialize the ImageManager with the driver
    //     // $manager = new ImageManager(['driver' => $driver]);
    //     // $driver = Config::get('images.driver');
    //     // $manager = new ImageManager(['driver' => Config::get('images.driver')]);
